...
func NewRedis() *redis.Client {
	c := redis.NewClient(&redis.Options{
		...
		Password: g.Config().Collector.Redis.Passowrd, // no password set
		DB:       0,                                   // use default DB
	})
	return c
}
...
func RedisMetrics() (L []*cmodel.MetricValue) {
	...
	tags = fmt.Sprintf("host=%s,port=%d",
		g.Config().Collector.Redis.Host,
		g.Config().Collector.Redis.Port)
	client := NewRedis()
	if client == nil {
		return
	}
...
func RedisStatInfo(client *redis.Client) (L []*cmodel.MetricValue) {
	monitorKeys := map[string]string{
		...
		"aof_rewrite_scheduled":          "GAUGE",   // AOF重写是否被RDB save操作阻塞等待0
	...
	for _, item := range strings.Split(r, "\n") {
		...
		if t, ok := monitorKeys[rkey[0]]; ok {
			if rkey[0] == "keyspace_hits" {
				...
				L = append(L, CounterValue("redis."+rkey[0], rkey[1], tags))
			} else if rkey[0] == "keyspace_misses" {
				keyspace_misses, _ = strconv.ParseFloat(rkey[1], 64)
				L = append(L, CounterValue("redis."+rkey[0], rkey[1], tags))
			} else if rkey[0] == "role" {
				if rkey[1] == "master" {
					L = append(L, CounterValue("redis."+rkey[0], "1", tags))
				} else {
					L = append(L, CounterValue("redis."+rkey[0], "0", tags))
				}
			} else if rkey[0] == "rdb_last_bgsave_status" {
				if rkey[1] == "ok" {
					L = append(L, GaugeValue("redis."+rkey[0], 1, tags))
				} else {
					L = append(L, GaugeValue("redis."+rkey[0], 0, tags))
				}
			} else if rkey[0] == "aof_last_bgrewrite_status" {
				if rkey[1] == "ok" {
					L = append(L, GaugeValue("redis."+rkey[0], 1, tags))
				} else {
					L = append(L, GaugeValue("redis."+rkey[0], 0, tags))
				}
			} else if rkey[0] == "aof_last_write_status" {
				if rkey[1] == "ok" {
					L = append(L, GaugeValue("redis."+rkey[0], 1, tags))
				} else {
					L = append(L, GaugeValue("redis."+rkey[0], 0, tags))
				}
			} else if rkey[0] == "redis_version" {
				redis_version = rkey[1]
			} else if t == "GAUGE" {
				L = append(L, GaugeValue("redis."+rkey[0], rkey[1], tags))
			} else if t == "COUNTER" {
				L = append(L, CounterValue("redis."+rkey[0], rkey[1], tags))
			}
		}
	...
	if keyspace_hits > 1.0 {
		...
		L = append(L, GaugeValue("redis.keyspace_hit_ratio", strconv.FormatFloat(keyspace_hit_ratio, 'f', 2, 64), tags))
	} else {
		L = append(L, GaugeValue("redis.keyspace_hit_ratio", "0.00", tags))
	}

	if redis_version > "2.6" {
		// L = append(L, RedisCmdStatInfo(client)...)
	}
